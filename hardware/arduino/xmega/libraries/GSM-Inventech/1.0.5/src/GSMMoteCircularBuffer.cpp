...
GSMMoteCircularBuffer::GSMMoteCircularBuffer(GSMMoteCircularBufferManager* mgr)
{
	head=0;
	tail=0;
	cbm=mgr;
...
int GSMMoteCircularBuffer::write(char c)
{
...
char GSMMoteCircularBuffer::read()
{
	char res;
	if(head!=tail)
	{
...
char GSMMoteCircularBuffer::peek(int increment)
{
	char res;
	byte num_aux;
...
void GSMMoteCircularBufferManager::spaceAvailable(){return;};

void GSMMoteCircularBuffer::flush()
{
	head=tail;
}

char* GSMMoteCircularBuffer::nextString()
{
	while(head!=tail)
	{
		head=(head+1) & __BUFFERMASK__;
...
bool GSMMoteCircularBuffer::locate(const char* reference)
{

	return locate(reference, head, tail, 0, 0);
}

bool GSMMoteCircularBuffer::chopUntil(const char* reference, bool movetotheend, bool usehead)
{
	byte from, to;
...
bool GSMMoteCircularBuffer::locate(const char* reference, byte thishead, byte thistail, byte* from, byte* to)
{
	int refcursor=0;
	bool into=false;
	byte b2, binit;
...
bool GSMMoteCircularBuffer::extractSubstring(const char* from, const char* to, char* buffer, int bufsize)
{
	byte t1;
	byte h2;
	byte b;
...
int GSMMoteCircularBuffer::readInt()
{
	int res=0;
	byte c;
	bool anyfound=false;
...
void GSMMoteCircularBuffer::debugBuffer()
{
	byte h1=head;
	byte t1=tail;
	Serial.println();
...
void GSMMoteCircularBuffer::printCharDebug(uint8_t c)
{
	if((c>31)&&(c<127))
		Serial.print((char)c);
	else
...
bool GSMMoteCircularBuffer::retrieveBuffer(char* buffer, int bufsize, int& SizeWritten)
{
	byte b;
	int i;
