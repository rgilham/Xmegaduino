...
*/
#ifndef __GSMMOTEHARDSERIAL__
#define __GSMMOTEHARDSERIAL__
...
class GSMMoteHardSerial : public GSMMoteCircularBufferManager {
	private:
	
	  
		static GSMMoteHardSerial* _activeObject;
		
		GSMMoteHardSerialMgr* mgr;
	  ...
	  public:
		...
		GSMMoteCircularBuffer cb; // Circular buffer

		/** Register serial manager
			@param manager		Serial manager
		 */
		inline void registerMgr(GSMMoteHardSerialMgr* manager){mgr=manager;};
		...
		/** Manage serial event
		 */
		static inline void handle_serialEvent();
