...
GSMMoteShieldV1BandManagement::GSMMoteShieldV1BandManagement(bool trace): modem(trace)
{
...
GSMMote_NetworkStatus_t GSMMoteShieldV1BandManagement::begin()
{
...
String GSMMoteShieldV1BandManagement::getBand()
{
  String modemResponse=modem.writeModemCommand("AT+QBAND?", 2000);
	
  for(GSMMoteGSMBand i=GSM850_EGSM_DCS_PCS_MODE;i>UNDEFINED;i=(GSMMoteGSMBand)((int)i-1))
  {
...
bool GSMMoteShieldV1BandManagement::setBand(String band)
{
	String command;
	...
	command="AT+QBAND=";
	for(GSMMoteGSMBand i=EGSM_MODE;((i<=GSM850_EGSM_DCS_PCS_MODE)&&(!found));i=(GSMMoteGSMBand)((int)i+1))
	{
