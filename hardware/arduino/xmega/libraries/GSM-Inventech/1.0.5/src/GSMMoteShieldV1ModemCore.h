...
#ifndef __GSMMote_SHIELDV1MODEMCORE__
#define __GSMMote_SHIELDV1MODEMCORE__
...
class GSMMoteShieldV1ModemCore : public GSMMoteHardSerialMgr , public Print
{
	private:
		...
		GSMMote_commandType_e ongoingCommand;
		...
		GSMMote_NetworkStatus_t _status;
		
		GSMMoteShieldV1BaseProvider* UMProvider[UMPROVIDERS];
		GSMMoteShieldV1BaseProvider* activeProvider;
	...
	public:
		...
		/** Constructor */
		GSMMoteShieldV1ModemCore();
		
		GSMMoteHardSerial gss; // Direct access to modem
		...
		/** Get ongoing command
			@return command
		 */
		GSMMote_commandType_e getOngoingCommand(){return ongoingCommand;};
		
		/** Set ongoing command
			@param c			 New ongoing command
		 */
		void setOngoingCommand(GSMMote_commandType_e c){ongoingCommand=c;};
		
		/** Open command
			@param activeProvider	Active provider
			@param c				Command for open
		 */
		void openCommand(GSMMoteShieldV1BaseProvider* activeProvider, GSMMote_commandType_e c);
		 ...
		 */
		inline GSMMoteCircularBuffer& theBuffer(){return gss.cb;};
		
		/** Establish a new network status
			@param status		Network status
		 */
		inline void setStatus(GSMMote_NetworkStatus_t status) { _status = status; };
		
		/** Returns actual network status
			@return network status
		 */
		inline GSMMote_NetworkStatus_t getStatus() { return _status; };
		...
		*/
		void registerUMProvider(GSMMoteShieldV1BaseProvider* provider);		
	
		/** unegister provider as willing to receive unsolicited messages
			@param provider	Pointer to provider able to receive unsolicited messages
		*/
		void unRegisterUMProvider(GSMMoteShieldV1BaseProvider* provider);		

		
		/** Register a provider as "dialoguing" talking in facto with the modem
			@param provider	Pointer to provider receiving responses
		*/
		void registerActiveProvider(GSMMoteShieldV1BaseProvider* provider){activeProvider=provider;};
...
extern GSMMoteShieldV1ModemCore theGSMMoteShieldV1ModemCore;
